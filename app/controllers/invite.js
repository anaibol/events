...
exports.checkInvitation = function(req, res) {
  graph.get('/invite/' + req.params.eventId + '/invited/' + req.params.userId + '?access_token=' + req.user.accessToken, function(err, result) {
    res.json(result.data);
  });
};

exports.sendInvitation = function(req, res) {
  graph.post('/' + req.params.eventId + '/invited/' + req.params.userId + '?access_token=' + req.user.accessToken, function(err, result) {
    if (err) {
      res.json(err);
    }
    else {
...
exports.sendInvitations = function(req, res) {
  graph.post('/' + req.params.eventId + '/invited/' + req.body.usersIds + '?access_token=' + req.user.accessToken, function(err, result) {
    if (err) {
      res.json(err);
    }
    else {
